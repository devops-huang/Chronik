 ...
 * 按「立春」口径取流年干支。
 *
 * 八字排盘以节气分年，流年在每年立春（约 2 月 3–5 日）交接，而非公历 1 月 1 日。
 * 原实现用 SixtyCycleYear.fromYear(年)，会把 1 月 1 日至立春前这段错误归入下一年。
 * 例：2027-01-15 原判丁未，实际仍属丙午年。
 */
function getLiunianGanzhiByLichun(date: Date): string {
  ...
  return scDay.getSixtyCycleMonth().getSixtyCycleYear().getSixtyCycle().toString();
}
...
function getFlowFromDate(
  ...
  const solarDay = SolarDay.fromYmd(date.getFullYear(), date.getMonth() + 1, date.getDate());
  const scDay = SixtyCycleDay.fromSolarDay(solarDay);
  // 流年以立春交接，不能使用公历年直接取干支
  const liunianGanzhi = getLiunianGanzhiByLichun(date);
...
function buildYearItems(
  ...
  for (let y = start.getFullYear(); y <= end.getFullYear(); y++) {
    // 以当年 7 月 1 日为基准取流年：此时必在该年立春之后，口径与立春分年一致
    const liunianGanzhi = getLiunianGanzhiByLichun(new Date(y, 6, 1));
    items.push({
      年份: y,
      流年干支: liunianGanzhi,
      流年十神: buildTenGodDetail(dayMaster, liunianGanzhi),

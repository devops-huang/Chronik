...
// ── P0-9 · 输出侧系统免责后缀（文案基准：docs/COMPLIANCE-WORDING.md §二）──
// 普通命理问答用 L2；经健康/投资/法律倾向识别标记高风险时用 L3（追加心理援助热线）。
export const CHAT_SUFFIX_L2 =
  '以上内容为传统命理文化的娱乐性解读，由 AI 生成，不构成医疗、法律、投资或任何专业建议。涉及健康、法律、财务等重大决策，请咨询对应持证专业人士。';
export const CHAT_SUFFIX_L3 =
  CHAT_SUFFIX_L2 + '\n如你正经历情绪危机，可联系全国心理援助热线 12356（以运营实际备案热线为准）。';

// 高风险倾向软识别（不硬拦，仅用于决定是否追加 L3 后缀）。覆盖健康 / 投资 / 法律三类倾向词。
const RISK_TENDENCY = {
  health: /健康|身体|疾病|医院|养生|作息|体检|睡眠|精力/,
  investment: /投资|理财|股票|基金|财务|财富|赚钱|生意|收入|开销/,
  legal: /法律|律师|合同|诉讼|纠纷|官司|维权/,
};
/**
 * 输出侧风险分级：命中健康/投资/法律倾向词返回 'L3'（追加心理援助后缀），否则 'L2'。
 * 与 isBlocked 的 A–J 硬禁区互不重叠——硬禁区已在服务端硬拦，不会流到此处。
 * @param {string} text
 * @returns {'L2'|'L3'}
 */
export function classifyRisk(text) {
  const t = normalize(text);
  for (const re of Object.values(RISK_TENDENCY)) {
    if (re.test(t)) return 'L3';
  }
  return 'L2';
}

/** 高风险倾向正则：健康 / 投资 / 法律（用于 L3 触发与 ai_audit.high_risk 标记）。 */
const HIGH_RISK_RE = /健康|生病|疾病|就医|受孕|怀孕|体质|症状|诊断|医院|中药|调理|手术|癌变|寿命|股票|基金|炒币|投资|理财|杠杆|重仓|全仓|暴富|官司|起诉|律师|遗嘱|继承|被告|家产|合同/;

/**
 * 判断文本是否呈现健康 / 投资 / 法律倾向（命中即标记 ai_audit.high_risk，并在流末追加 L3 后缀）。
 * 命中基于"倾向关键词"，不做确定性断言判断，避免"我明年适合换工作吗"等正常问答被误杀（架构裁决 §4.3）。
 */
export function isHighRisk(text) {
  return HIGH_RISK_RE.test(normalize(text));
}

...
import { buildPersonalFortune } from './fortuneEngine.js';

/**
 * 建除十二神当日宜忌（标准黄历）。
 * 修复：cantian-tymext 0.0.26 的 LunarDay.getRecommends()/getAvoids() 返回的是重叠的宜向数据
 * （getAvoids 的 700 项全部包含于 getRecommends 的 1820 项中，内容都是「祭祀、祈福」类宜项），
 * 导致前端宜、忌两栏显示相同；Taboo 类在本版亦 broken。故以建除神自行映射，确保宜忌互补且不重叠。
 */
const DUTY_YI_JI = {
  建: { yi: ['出行', '祈福', '动土', '订盟', '纳财', '赴任'], ji: ['开仓', '乘船', '动工', '词讼'] },
  除: { yi: ['祭祀', '祈福', '解除', '疗病', '移徙', '出行', '扫舍'], ji: ['求官', '上任', '嫁娶'] },
  满: { yi: ['祭祀', '祈福', '嫁娶', '移徙', '开市', '会友'], ji: ['动土', '安葬', '入宅', '词讼'] },
  平: { yi: ['修造', '嫁娶', '移徙', '安床', '交易'], ji: ['词讼', '出行', '安葬'] },
  定: { yi: ['祭祀', '祈福', '嫁娶', '造屋', '入学', '安床'], ji: ['词讼', '出行', '求医', '动土'] },
  执: { yi: ['造屋', '修造', '嫁娶', '收购', '纳财'], ji: ['开市', '移徙', '出行', '签约'] },
  破: { yi: ['破屋', '坏垣', '求医', '治病'], ji: ['嫁娶', '出行', '签约', '动土', '开市', '安葬'] },
  危: { yi: ['安床', '祭祀', '祈福', '交易'], ji: ['登高', '出行', '乘船', '移徙'] },
  成: { yi: ['嫁娶', '开市', '入学', '安床', '动土', '会友'], ji: ['词讼', '争斗', '出行'] },
  收: { yi: ['嫁娶', '纳财', '收购', '入仓', '安床'], ji: ['放债', '出行', '安葬', '开市'] },
  开: { yi: ['祭祀', '祈福', '入学', '开市', '动土', '出行'], ji: ['安葬', '放债', '词讼'] },
  闭: { yi: ['安葬', '入殓', '补垣', '筑堤', '迁坟'], ji: ['开市', '出行', '求医', '手术', '嫁娶'] },
};
 ...
 * @param {{dayMasterElement:string}|null} user  旧版二元所需（日主五行）
 * @param {string} todayStr
 * @param {{dayStem:string,monthZhi:string,gender?:number}|null} natal  5.3 四元所需的命盘核心
 */
export function buildTodayFortune(user, todayStr, natal = null) {
  ...
  // 5.3：四元个性化运势（日主天干 × 月令 × 流日天干 × 流日地支）。
  // 无命盘数据时返回 null，前端降级展示上面的通用 headline，不会报错。
  const personal = buildPersonalFortune(natal, todayStr);

  let yi = [], ji = [];
  try {
    const dutyName = lunar.getDuty()?.getName?.();
    const map = DUTY_YI_JI[dutyName];
    if (map) { yi = map.yi; ji = map.ji; }
  } catch {}
